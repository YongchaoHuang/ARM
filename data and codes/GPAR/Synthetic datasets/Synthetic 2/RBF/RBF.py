...
output_dir = "~/Synthetic datasets/Synthetic 2"
...
# Convert transactions to binary matrix for Apriori, FP-Growth, and Eclat
...
# GPAR: Log-likelihood for GP parameter estimation
def neg_log_likelihood(length_scale, X, transactions):
    K = rbf_kernel(X, length_scale) + 1e-6 * np.eye(num_items)
    log_lik = 0
...
# GPAR: Co-occurrence probability
...
# Functions for analysis (added from reference code)
...
min_support_values = [0.1, 0.15, 0.2, 0.25, 0.3]  # Adjusted to lower values
min_conf = 0.3  # Lowered to allow more rules
...
initial_l = 1.0
result = minimize(neg_log_likelihood, initial_l, args=(X, transactions), bounds=[(0.1, 100)])
length_scale = result.x[0]
print(f'Estimated length scale: {length_scale}')
...
for min_support in tqdm(min_support_values, desc="Processing min_support values"):
    ...
    # Save GPAR summary table for this min_support
    gpar_summary_data = []
    ...
    if apriori_rules.empty:
        print(f"No Apriori rules found for min_support={min_support}")
        apriori_runtimes.append(end_time - start_time)
        apriori_memory.append(max(end_memory - start_memory, 0))
        apriori_num_rules.append(0)
        apriori_num_itemsets.append(len(apriori_freq))

        # Save an empty Apriori summary table
        apriori_summary_data = []
        apriori_summary_df = pd.DataFrame(apriori_summary_data, columns=['#', 'Premises', 'Conclusion', 'Support', 'Confidence', 'Lift'])
        apriori_summary_df.to_csv(os.path.join(output_dir, f"apriori_summary_min_support_{min_support}.csv"), index=False)

        # Apriori Analysis (empty)
        apriori_analysis_data = []
        print(f"\n Top {top_n} Apriori rules with item {RHS_item_no} on RHS (min_support={min_support}): None")
    else:
        apriori_runtimes.append(end_time - start_time)
        apriori_memory.append(max(end_memory - start_memory, 0))
    ...
    # Handle empty rules
    if fpgrowth_rules.empty:
        print(f"No FP-Growth rules found for min_support={min_support}")
        fpgrowth_runtimes.append(end_time - start_time)
        fpgrowth_memory.append(max(end_memory - start_memory, 0))
        fpgrowth_num_rules.append(0)
        fpgrowth_num_itemsets.append(len(fpgrowth_freq))

        # Save an empty FP-Growth summary table
        fpgrowth_summary_data = []
        fpgrowth_summary_df = pd.DataFrame(fpgrowth_summary_data, columns=['#', 'Premises', 'Conclusion', 'Support', 'Confidence', 'Lift'])
        fpgrowth_summary_df.to_csv(os.path.join(output_dir, f"fpgrowth_summary_min_support_{min_support}.csv"), index=False)

        # FP-Growth Analysis (empty)
        fpgrowth_analysis_data = []
        print(f"\n Top {top_n} FP-Growth rules with item {RHS_item_no} on RHS (min_support={min_support}): None")
    else:
        fpgrowth_runtimes.append(end_time - start_time)
        fpgrowth_memory.append(max(end_memory - start_memory, 0))
    ...
    # Handle empty rules
    if eclat_rules.empty:
        print(f"No Eclat rules found for min_support={min_support}")
        eclat_runtimes.append(end_time - start_time)
        eclat_memory.append(max(end_memory - start_memory, 0))
        eclat_num_rules.append(0)
        eclat_num_itemsets.append(len(eclat_freq))

        # Save an empty Eclat summary table
        eclat_summary_data = []
        eclat_summary_df = pd.DataFrame(eclat_summary_data, columns=['#', 'Premises', 'Conclusion', 'Support', 'Confidence', 'Lift'])
        eclat_summary_df.to_csv(os.path.join(output_dir, f"eclat_summary_min_support_{min_support}.csv"), index=False)

        # Eclat Analysis (empty)
        eclat_analysis_data = []
        print(f"\n Top {top_n} Eclat rules with item {RHS_item_no} on RHS (min_support={min_support}): None")
    else:
        eclat_runtimes.append(end_time - start_time)
        eclat_memory.append(max(end_memory - start_memory, 0))
    ...
    # Save key numbers (metrics) for this iteration
    metrics_data = {
    ...
    # Plotting and saving figures for this iteration
