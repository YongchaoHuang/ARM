...
# Neural Tangent Kernel (NTK1) for a 3-layer neural network with ReLU activation
def neural_tangent_kernel(X, sigma_f):
    """
    Neural Tangent Kernel for a 3-layer neural network (input, hidden, output) with ReLU activations.
    Formula derived as:
    k^{NTK}(x, x') = m * { (||x|| ||x'|| / (2π)) (sqrt(1 - ρ^2) + ρ arccos(-ρ)) + (1/4 + arcsin(ρ)/(2π)) x^T x' }
    where ρ = (x^T x') / (||x|| ||x'||).
    Parameters:
    - X: Feature matrix (M x d)
    - sigma_f: Signal variance parameter (replaces m for optimization)
    Returns:
    - Kernel matrix K (M x M)
    """
    # Compute dot products and norms
    dot_prods = X @ X.T  # Shape: (M x M)
    norms = np.sqrt(np.sum(X ** 2, axis=1))  # Shape: (M,)
    norms_i = norms[:, None]  # Shape: (M x 1)
    norms_j = norms[None, :]  # Shape: (1 x M)

    # Compute ρ = (x^T x') / (||x|| ||x'||)
    denom = norms_i * norms_j  # Shape: (M x M)
    # Avoid division by zero
    denom[denom == 0] = 1.0
    rho = dot_prods / denom
    rho = np.clip(rho, -1, 1)  # Ensure ρ is in [-1, 1] for trigonometric functions

    # Compute the two terms of NTK1
    # Term 1: (||x|| ||x'|| / (2π)) (sqrt(1 - ρ^2) + ρ arccos(-ρ))
    term1 = (norms_i * norms_j / (2 * np.pi)) * (np.sqrt(1 - rho**2) + rho * np.arccos(-rho))

    # Term 2: (1/4 + arcsin(ρ)/(2π)) x^T x'
    term2 = (1/4 + np.arcsin(rho) / (2 * np.pi)) * dot_prods

    # Combine terms and scale by sigma_f (replacing m)
    K = sigma_f * (term1 + term2)
    return K

# GPAR: Log-likelihood for GP parameter estimation (Updated for NTK1)
def neg_log_likelihood(sigma_f, X, transactions):
    K = neural_tangent_kernel(X, sigma_f) + 1e-6 * np.eye(num_items)  # Add jitter for numerical stability
    log_lik = 0
...
# Optimize sigma_f for GPAR once (Updated for NTK1)
initial_sigma_f = 1.0  # Initial value for sigma_f
bounds = [(0.1, 100)]  # Bounds for sigma_f
result = minimize(neg_log_likelihood, initial_sigma_f, args=(X, transactions), bounds=bounds)
sigma_f = result.x[0]
print(f'Estimated sigma_f: {sigma_f}')
K = neural_tangent_kernel(X, sigma_f)
